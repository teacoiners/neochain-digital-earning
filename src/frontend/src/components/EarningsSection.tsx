...
export default function EarningsSection() {
  ...
  const PAID_SPIN_COST = 30;

  const handleSpin = async (paid = false) => {
    if (isSpinning) return;
    if (!paid && !freeSpinAvailable) return;
    if (!identity) {
    ...
    if (paid) {
      ...
      if (bal < PAID_SPIN_COST) {
        toast.error(
          `Insufficient balance! You need ₹${PAID_SPIN_COST} to spin.`,
        );
        return;
      }
    }
          ...
          >
            ...
            {freeSpinAvailable ? (
              ...
              <div className="flex flex-col gap-2 w-full">
                {nextSpinCountdown && (
                  <div
                    className="w-full py-2 rounded-xl text-center text-sm"
                    style={{
                ...
                <button
                  ...
                  disabled={isSpinning}
                  className="w-full py-3.5 rounded-xl font-display font-black text-base transition-all disabled:opacity-60"
                  style={{
                    background: isSpinning
                      ? "rgba(38,214,255,0.08)"
                      : "linear-gradient(135deg, rgba(38,214,255,0.25) 0%, rgba(123,77,255,0.3) 100%)",
                    border: "1px solid rgba(38, 214, 255, 0.5)",
                    boxShadow: isSpinning
                      ? "none"
                      : "0 0 20px rgba(38,214,255,0.3), 0 4px 12px rgba(123,77,255,0.2)",
                    color: "oklch(0.9 0.15 200)",
                    letterSpacing: "0.05em",
                  }}
                  data-ocid="earnings.button.paid"
                >
                  {isSpinning
                    ? "⏳ Spinning..."
                    : `⚡ Spin for ₹${PAID_SPIN_COST}`}
                </button>
              </div>
            )}
          </motion.div>
